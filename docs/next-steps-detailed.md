...
### Known: ppca64 EExternalException on exit

On Windows arm64, **ppca64.exe -iV** can print the version (e.g. 3.3.1) and then raise **EExternalException** on exit (RTL/SEH shutdown path). CI “Verify ppca64” treats the step as success if the captured output contains the expected version; follow-up is to fix the exit-path crash in the RTL/compiler.

**Why this happens:** The RTL raises EExternalException when Windows delivers an exception whose **exception code is not in the RTL’s known list** (access violation, divide-by-zero, etc.). See `rtl/win/sysutils.pp` (`WinExceptionObject` / `FindExceptMapEntry`). So during ppca64 shutdown, something triggers a Windows exception with an unrecognized code (possibly ARM64/SEH-unwind related). **Do others see it?** There are no public reports of “ppca64 EExternalException on exit” for aarch64-win64; FPC’s Windows exception handling has a long history (e.g. [FPC #12974](https://gitlab.com/freepascal.org/fpc/source/-/issues/12974) – SEH in DLLs, fixed for x86_64 in 3.2.0). Native **aarch64-win64** is new (this bounty), so this may be an ARM64-specific RTL/SEH bug (e.g. finalization or unwind). **PowerShell:** The `System.Management.Automation.RemoteException` in the log is PowerShell surfacing the child process (ppca64) failure. **Next steps:** Reproduce under a debugger on Windows arm64 to see the exact exception code; consider reporting to FPC with that code and a minimal “ppca64 -iV then exit” scenario.
